...
def register(user, name):
    ...
    if r.status_code == 500 and INCOMPATIBILITY_MSG in r.text:
        print("Shield no longer accepts CSRF with cookies")
        exit()
...
with requests.Session() as s:
    ...
    assert f'Form successfully submitted as user {NAME2}' in r_attacker.text, " NOT VULNERABLE to post-login CSRF attack"
