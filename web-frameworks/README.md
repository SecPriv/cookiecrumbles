...
* an attacker application that performs the attack to the respective target application.
...
1. select the protocol to be tested, `http` or `https` (defaults to http) via
...
All artifacts can be built and run using `docker-compose`, providing some environment variables (details on these variables can be found in the table below). As an example, to test the CORF Token Fixation attack (pre-login) on express, one should use on the `express-pre-login` folder:

```bash
$ export VERSION="v0.5.3"; docker-compose up -d --build
```

This will start the artifact with the server listening on ports 80 and 443.

The testing scripts for each framework have been tested on a standard Ubuntu 22.04.2 LTS running Python 3.10.6. The only dependencies are the `requests` and `Beautiful Soup` libraries, which can be installed using `pip install --user requests bs4`. These tests are run as:
...
```bash
Should be vulnerable to pre-login
[+] Testing login
...
* The testing script performs the attack by transferring 200 credits to the `attacker` on behalf of `alice`.
...
```bash
export VERSION="v0.6.0"; docker-compose up -d --build
...
and observe that the session cookie at step 3. is different from the session cookie at step 1.
...
express | express-session-fixation | Session Fixation | VERSION="v0.5.3"                 | CVE-2022-25896
express | express-session-fixation | Session Fixation | VERSION="v0.6.0"                 | Fixed
express | express-session-fixation | Session Fixation | VERSION="v0.6.0-keepSessionInfo" | Fixed
...
symfony | symfony-pre-login        | Pre-login        | VERSION="v5.4.19"                | CVE-2022-24895
symfony | symfony-pre-login        | Pre-login        | VERSION="v5.4.20"                | Fixed
...
Each application has a script to automatically perform the test. Users can however test the applications manually following similar steps to the ones described above for the `express` framework.
...
The goal is to perform a CORF token fixation attack and transfer money to the attacker's account. Exceptions to this are the following where we performed modifications to demo applications provided with the frameworks:

* tornado framework has a demo blog application where the CSRF protected operation is the creation of posts. In this case an attacker is able to create posts on behalf of the authenticated user. As a side note, new users can be created using endpoint `auth/create`.
...
Also, while manually testing, fastify and codeigniter 4 present non-standard testing flows:
...
* for codeigniter4, the attack flow for the `regenerate` case is also non-standard; for this attack to work one needs to set the pre-session (in attack.localtest.me), login in the application, and then evict the old cookie (or let it expire after 20s), get a fresh seed, and only afterwards perform the CORF attack.
