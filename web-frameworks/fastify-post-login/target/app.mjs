...
// CSRF: Use with @fastify/cookie signed cookies:
...
// PASSPORT-OPTION1: use with @fastify/session
...
// // PASSPORT-OPTION2: use with @fastify/secure-session
...
fastifyPassport.registerUserSerializer(async (user, request) => { return user; } );
...
fastify.route({
    ...
    handler: async (req, reply) => {
      ...
      return reply.view("/views/index.ejs", {
        ...
        debug_csrf_secret : req.cookies._csrf,       // for CSRF-cookies
        csrf_token: token,
        })
    }
})
...
fastify.post('/login',
    { preValidation: fastifyPassport.authenticate('local', { successRedirect: '/', failureRedirect: '/login', authInfo: false }) },
    (req, res, next) => {
...
fastify.post(
  ...
  async (req, reply) => {
    if( !(req.isAuthenticated())){
        ...
        return "Successfully transferred " + req.body.ammount + " from " + req.user + " to " + req.body.target;
    }
  }
)
